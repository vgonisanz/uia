...
export CURRENT_VERSION:=$(shell cat ${ROOTDIR}/uia/_meta.py  | sed -En "s/^__version__\s*=\s*[ubrf]*['\"]([^'\"]+)['\"].*$$/\1/p")
...
uninstall: ## uninstall the package
	pip uninstall -y $(PIP_ARGS) uia || true
...
lint: ## static code analysis with pylint
	pylint --rcfile uia/.pylintrc -j 0 uia
...
code-locs: ## display metrics (LOCs, number of comments, etc.)
	radon raw --summary uia

code-complexity: ## check cyclomatic complexity using radon
	xenon --max-absolute B --max-modules A --max-average A uia
...
security: ## check source code for vulnerabilities
	@[ "${REPORT_FORMAT}" ] && ( mkdir -p docs/_build/security && bandit -v -r -f ${REPORT_FORMAT} -o docs/_build/security/index.html uia &> /dev/null ) || true
	bandit -v -r uia
